...
describe('generateShopLayout', () => {
  ...
  it('R60②: 全6種のパークが全ステージに配置される(ダブルタップ等の欠落なし)', () => {
    const ALL = ['juggernog', 'speed-cola', 'double-tap', 'stamin-up', 'quick-revive', 'ext-mag'];
    for (const seed of [163, 167, 173, 179, 181, 191, 193, 197, 199, 211]) {
      const perks = generateShopLayout(seed)
        .slots.filter((s) => s.kind === 'perk-machine')
        .map((s) => s.perkId);
      expect(perks.length).toBe(6); // 3〜4台の抽選 → 全6種必置へ
      for (const id of ALL) expect(perks).toContain(id);
    }
  });
...
describe('generateShopLayout: pack-a-punch/door 追加後も既存スロットはビット不変', () => {
  ...
  // R60②: パーク自販機は「抽選3〜4台」から「全6種必置」へ意図的に変更(下のR60②テストで担保)。
  // 従ってスナップショット完全一致は壁武器サブ列に絞る — rand消費を1個維持したことで壁武器の
  // 選出・順序・slotIndex はR53-W2着手前とビット単位で不変であることを引き続き保証する。
  it('壁武器の内容・順序はR53-W2着手前と完全一致(R60②の全パーク配置後もrand消費列不変)', () => {
    for (const seedStr of Object.keys(PRE_R53W2_SNAPSHOT)) {
      const seed = Number(seedStr);
      const beforeWalls = PRE_R53W2_SNAPSHOT[seedStr]!.filter((s) => s.kind === 'wall-buy');
      const afterWalls = generateShopLayout(seed).slots.filter((s) => s.kind === 'wall-buy');
      expect(afterWalls).toEqual(beforeWalls);
    }
  });
