...
describe('CAMPAIGN 構造(全11章/61ミッション、R53-W2でch9/ch10「帝王編」、R54-F6で隠し章chB「歴戦の間」を追加)', () => {
  it('11章 / ch1-10は各6ミッション+chBは1ミッション / 合計61', () => {
    expect(CAMPAIGN).toHaveLength(11);
    for (const c of CAMPAIGN) {
      expect(c.missions).toHaveLength(c.id === 'chB' ? 1 : 6);
    }
    expect(flat()).toHaveLength(61);
  });

  it('chapter.id は ch1..ch10, chB で順に並ぶ(chB末尾=nextChapterId連鎖でch10制覇後に解放)', () => {
    expect(CAMPAIGN.map((c) => c.id)).toEqual([
      'ch1', 'ch2', 'ch3', 'ch4', 'ch5', 'ch6', 'ch7', 'ch8', 'ch9', 'ch10', 'chB',
    ]);
  });
...
describe('stageId / バイオーム', () => {
  ...
  it('章ごとに想定 biome を使う(ch9=urban/ch10=neon)', () => {
    const expected: Record<string, string> = {
      ...
      ch9: 'urban',
      ch10: 'neon',
      chB: 'industrial', // R54-F6: 歴戦の間=演算炉(工廠系)
    };
    for (const c of CAMPAIGN) {
...
describe('支給武器 / 章ごとの primaryId(ch10 は c10m6 のみ fists へ上書き)', () => {
  it('各章の全ミッションが規定の支給武器を持つ(章の既定値からの上書きを除く)', () => {
    const expected: Record<string, string> = {
      ...
      ch9: 'akatsuki-ar',
      ch10: 'raitei-lmg',
      chB: 'raitei-lmg', // R54-F6: 歴戦の間(ブリーフィングで持ち替え可能な既定支給)
    };
...
describe('waves / objective 不変条件(61ミッション全体)', () => {
...
describe('クエリ関数(61ミッション)', () => {
  it('allMissions は61件で CAMPAIGN を平坦化したもの', () => {
    expect(allMissions()).toHaveLength(61);
  ...
  it('ch10最終ミッションの次は chB歴戦の間(R54-F6 隠し章の連結)', () => {
    expect(nextMissionId('c10m6-kurogane-throne')).toBe('cbm1-rekisen');
  });

  it('nextMissionId を辿ると全61ミッションを順に巡れる(末尾はchBの歴戦の間)', () => {
    const order: string[] = [];
    ...
    expect(order[order.length - 1]).toBe('cbm1-rekisen');
  });
});
...
describe('MissionDef.challenge(3つ目の★条件・R54-W2 P0-A)', () => {
  it('全61ミッションが1個ずつ challenge を持つ(3★到達不能の根治)', () => {
  ...
  it('61ミッションの challenge 一覧表(id×kind×valueの回帰スナップショット)', () => {
    ...
    const expected: Record<string, { kind: MissionChallengeKind; value?: number }> = {
      ...
      // R54-F6: 隠し章chB(歴戦の間ボスラッシュ)
      'cbm1-rekisen': { kind: 'hs-count', value: 10 },
    };
    expect(Object.keys(expected)).toHaveLength(61);
    for (const m of allMissions()) {
...
describe('chB 歴戦の間(R54-F6 ボスラッシュ)', () => {
  const chB = CAMPAIGN.find((c) => c.id === 'chB')!;
  const m = chB.missions[0]!;

  it('1ミッション(cbm1-rekisen)のみ・index=0・industrial系ステージ', () => {
    expect(chB.missions).toHaveLength(1);
    expect(m.id).toBe('cbm1-rekisen');
    expect(m.index).toBe(0);
    expect(m.stageId).toMatch(/^gen-industrial-\d+$/);
  });

  it('6波の連戦: 先頭start+残り5波は全てwave-clear', () => {
    expect(m.waves).toHaveLength(6);
    expect(m.waves[0]!.trigger).toBe('start');
    for (const w of m.waves.slice(1)) expect(w.trigger).toBe('wave-clear');
  });

  it('各波はtier=bossの単独1体(bossHp01/ボスバーが波ごとに一意に立つ)', () => {
    for (const w of m.waves) {
      expect(w.enemies).toHaveLength(1);
      expect(w.enemies[0]!.tier).toBe('boss');
      expect(w.enemies[0]!.count).toBe(1);
      expect((w.enemies[0]!.name ?? '').length).toBeGreaterThan(0);
    }
  });

  it('6連戦の構成: tank→drone→giant→master(2段)→zombie→master(3段=クロガネ再来)', () => {
    expect(m.waves.map((w) => w.enemies[0]!.kind)).toEqual([
      'tank', 'drone', 'giant', 'master', 'zombie', 'master',
    ]);
    expect(m.waves.map((w) => w.enemies[0]!.hpMul)).toEqual([3, 3, 2, 4, 5, 10]);
  });

  it('resupply(補給+8秒小休止)は2波目以降の全wave-clear波に付与、start波には無い', () => {
    expect(m.waves[0]!.resupply).toBeUndefined();
    for (const w of m.waves.slice(1)) expect(w.resupply).toBe(true);
  });

  it('剣士=2段/クロガネ再来=3段フェーズ、いずれもhp01降順(フェーズ機の契約)', () => {
    const swordsman = m.waves[3]!.enemies[0]!.phases!;
    const kurogane = m.waves[5]!.enemies[0]!.phases!;
    expect(swordsman).toHaveLength(2);
    expect(kurogane).toHaveLength(3);
    for (const phases of [swordsman, kurogane]) {
      for (let i = 1; i < phases.length; i += 1) {
        expect(phases[i]!.hp01).toBeLessThan(phases[i - 1]!.hp01);
      }
    }
    // クロガネ再来はc10m6と同構成(0.6=黒斬撃+ブリンク、0.25=雷柱+召喚3)
    expect(kurogane[1]!.hp01).toBe(0.6);
    expect(kurogane[1]!.blackSlash).toBe(true);
    expect(kurogane[1]!.blink).toBe(true);
    expect(kurogane[2]!.hp01).toBe(0.25);
    expect(kurogane[2]!.pillars).toBe(true);
    expect(kurogane[2]!.summonCount).toBe(3);
  });

  it('objective=eliminate-all / par300秒 / チャレンジ=hs-count10 / 無線4本', () => {
    expect(m.objective.kind).toBe('eliminate-all');
    expect(m.parTimeS).toBe(300);
    expect(m.durationS).toBe(600);
    expect(m.challenge).toEqual({ kind: 'hs-count', value: 10, label: expect.any(String) });
    expect(m.radio).toHaveLength(4);
  });

  it('最終戦のボス名は「クロガネ」を含む(story-engineのクロガネ演出判定が名前ベースで発火する)', () => {
    expect(m.waves[5]!.enemies[0]!.name).toContain('クロガネ');
  });
});
