...
describe('resolveNaturalBotKind (R51 ユーザー⑥: 個人戦の達人/巨躯自然湧き除外)', () => {
  ...
  it('hellMode ON かつ個人戦(teamBased=false): 巨躯は30/35%で不変、達人はR60①で精鋭(hard)時のみ', () => {
    // giant帯[0.30,0.35)は難易度に依らず不変(頻度を上げない)
    expect(resolveNaturalBotKind(() => 0.32, false, true, false, 'hard')).toBe('giant');
    expect(resolveNaturalBotKind(() => 0.32, false, true, false, 'normal')).toBe('giant');
    expect(resolveNaturalBotKind(() => 0.9, false, true, false, 'hard')).toBe('humanoid');
    // master帯[0,0.30): hard のみ master、それ以外は humanoid
    expect(resolveNaturalBotKind(() => 0.1, false, true, false, 'hard')).toBe('master');
    expect(resolveNaturalBotKind(() => 0.1, false, true, false, 'normal')).toBe('humanoid');
    expect(resolveNaturalBotKind(() => 0.1, false, true, false, 'easy')).toBe('humanoid');
  });

  it('hellMode ON かつチーム戦(teamBased=true): 達人は精鋭時のみ・巨躯は不変', () => {
    expect(resolveNaturalBotKind(() => 0.1, true, true, false, 'hard')).toBe('master');
    expect(resolveNaturalBotKind(() => 0.1, true, true, false, 'normal')).toBe('humanoid');
    expect(resolveNaturalBotKind(() => 0.32, true, true, false, 'normal')).toBe('giant');
  });
  ...
  it('トグルOFF かつチーム戦: 達人[0,0.08)は精鋭時のみ・巨躯[0.08,0.13)は難易度不変(R60①)', () => {
    // master帯: hard のみ master、normal/easy は humanoid
    expect(resolveNaturalBotKind(() => 0.05, true, false, false, 'hard')).toBe('master');
    expect(resolveNaturalBotKind(() => 0.05, true, false, false, 'normal')).toBe('humanoid');
    expect(resolveNaturalBotKind(() => 0.05, true, false, false, 'easy')).toBe('humanoid');
    // giant帯: 難易度に依らず一定(ユーザー「巨躯はそのまま」= 頻度を上げない)
    expect(resolveNaturalBotKind(() => 0.10, true, false, false, 'hard')).toBe('giant');
    expect(resolveNaturalBotKind(() => 0.10, true, false, false, 'normal')).toBe('giant');
    expect(resolveNaturalBotKind(() => 0.10, true, false, false, 'easy')).toBe('giant');
    expect(resolveNaturalBotKind(() => 0.5, true, false, false, 'normal')).toBe('humanoid');
  });

  it('R60①: difficulty 省略(既定normal)では master 帯が humanoid になる(後方互換=精鋭以外は達人なし)', () => {
    expect(resolveNaturalBotKind(() => 0.05, true, false, false)).toBe('humanoid');
    expect(resolveNaturalBotKind(() => 0.1, false, true, false)).toBe('humanoid');
  });
});
