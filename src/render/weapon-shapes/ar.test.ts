...
// (3)光学無=アイアン単ドット(carryHandle 機は 0.116 / 他は 0.075)で resolveSightY と一致、
...
// 9挺: [weaponId, modelKey, 期待アイアン狙点Y]。carryHandle 機(FAMAS/SG550)は 0.116。
const AR9: Array<[string, string, number]> = [
  ['kaede-ar', 'ar-famas', 0.116],
  ['miyama-br', 'ar-fal', 0.075],
  ['kasasagi-ar', 'ar-scar-h', 0.075],
  ['mukudori-br', 'ar-scar-l', 0.075],
  ['tobikuma-ar', 'ar-hk416', 0.075],
  ['ginyanma-ar', 'ar-mcx', 0.075],
  ['shinonome-ar', 'ar-arx', 0.075],
  ['kagerou-br', 'ar-sg550', 0.116],
  ['akatsuki-ar', 'bullpup', 0.075],
];
...
describe('R58 AR系 実在シルエット化', () => {
  ...
  it('FAMAS/SG-512 は carryHandle を立て、狙点が IRON_POST_Y より高い(内蔵サイト)', () => {
    expect(AR_SHAPES['ar-famas'].carryHandle).toBe('famas');
    expect(AR_SHAPES['ar-sg550'].carryHandle).toBe('ar15');
    expect(resolveSightY(def('kaede-ar'))).toBeCloseTo(0.116, 6);
    expect(resolveSightY(def('kagerou-br'))).toBeCloseTo(0.116, 6);
    expect(resolveSightY(def('kaede-ar'))).toBeGreaterThan(0.075);
  });
