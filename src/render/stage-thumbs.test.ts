...
describe('R64 imagegenステージカード対応', () => {
  it('固定ステージは全て静的WebP URLへ対応し、未知IDだけフォールバックする', async () => {
    const { STATIC_STAGE_THUMB_IDS, staticStageThumbUrl } = await import('./stage-thumbs');
    const { STAGES } = await import('../game/stages');

    expect([...STAGES].every((stage) => STATIC_STAGE_THUMB_IDS.has(stage.id))).toBe(true);
    expect(STATIC_STAGE_THUMB_IDS.size).toBe(STAGES.length);
    for (const stage of STAGES) {
      expect(staticStageThumbUrl(stage.id)).toMatch(
        new RegExp(`/assets/stage-thumbs/${stage.id}\\.webp$`),
      );
    }
    expect(staticStageThumbUrl('campaign-generated-unknown')).toBeNull();
  });
});
