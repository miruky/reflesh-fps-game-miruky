...
function getShared(): SharedMats {
  if (!sharedMats) {
    ...
    const sleeve = clothMat(0x536047, 0.9);
...
const FIST_POSES: FistPose[] = [
  // 柄と右手の相対位置を保ったまま、刃先だけを左上へ向ける。
  { name: FIST_KUNAI, rest: { p: [0.02, -0.05, 0], r: [-0.12, 0.06, 0] }, ads: { p: [0.07, -0.075, -0.01], r: [-0.22, 0.38, -0.26] } },
  { name: 'vm:fistRArm', rest: { p: [0.08, -0.2, 0.12], r: [0.5, -0.12, 0] }, ads: { p: [0.08, -0.19, 0.11], r: [0.46, -0.04, -0.08] } },
  { name: 'vm:fistRHand', rest: { p: [0.02, -0.07, -0.09], r: [0.2, 0.05, 0] }, ads: { p: [0.07, -0.095, -0.1], r: [0.26, 0.12, -0.14] } },
  // 守り手は左下へ開き、斬撃・突きへ移れる余白と中央視界を作る。
  { name: 'vm:fistLArm', rest: { p: [-0.1, -0.16, -0.02], r: [0.42, 0.24, 0.1] }, ads: { p: [-0.12, -0.18, -0.04], r: [0.44, 0.2, 0.08] } },
  { name: 'vm:fistLHand', rest: { p: [-0.11, -0.09, -0.16], r: [0.25, 0.1, 0] }, ads: { p: [-0.13, -0.12, -0.13], r: [0.3, 0.16, 0.04] } },
];
